...
function timerStart(){//Starts the timer on activation
    let interval = Math.floor(Math.random() * (21 - 2)) + 2//Picks a random number between 20 and 2, and rounds it down.
...
function displayButton(){//Displays the button on activation
    ...
    theButton.innerHTML += `
    <button class="t3button" onclick="ignoreTrap(), buttonClick()">Click Me!</button>
    `
    timer = setTimeout(gameLost, 1000)//Sets an activation timer for 2000 miliseconds, equating to 1 second. After 1 second has passed, activate gameLost function
}
