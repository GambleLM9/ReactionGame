...
function timerStart(){//Starts the timer on activation
    let interval = Math.floor(Math.random() * (16 - 3)) + 3//Picks a random number between 15 and 3, and rounds it down.
...
function displayButton(){//Displays the button on activation
    ...
    theButton.innerHTML += `
    <button class="t2button" onclick="ignoreTrap(), buttonClick()">Click Me!</button>
    `
    timer = setTimeout(gameLost, 1500)//Sets an activation timer for 2000 miliseconds, equating to 1 and a half seconds. After those seconds have passed, activate gameLost function
}
